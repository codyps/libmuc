...
const static uint8_t channels [] = {0, 1, 2, 3}; //Actual channels, indices are virtuals, left to right.

// ADC clock calculation
#define ADC_MAX_CLK 200000
#define ADC_PRESCALE  (ceil(log(F_CPU/ADC_MAX_CLK)/log(2)))
#define ADC_CLK (F_CPU/pow(2,ADC_PRESCALE)) //Hz
#define ADC_CYCLE (F_CPU/ADC_CLK) // CPU cycles per ADC cycle
#define DELAY_OPS 3 //_delay_loop_1
//#define DELAY_OPS 4 //_delay_loop_2
#define ADC_CYCLE_DELAY (ADC_CYCLE/DELAY_OPS+!!((int)ADC_CYCLE%(int)DELAY_OPS))
// F_CPU=2MHz,PRESCALE=4,clkadc=125kHz=125000
//	 8Mhz,	       6,	125kHz=125000
